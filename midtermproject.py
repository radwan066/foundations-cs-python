...
def Switch_Tabs(): #first printing the list of the opened tabs than asking the user for the index of the tab in which he would see its content O(n^2)
    print(openedtabs)
    y=(input("please,enter the index of the tabs you want to display its content:"))
    if y.isdigit(): #O(n) n:invalid y
     index=int(y)
     if openedtabs[index] in openedtabs:   #searching for the index given by the user O(n) n:length of the list
...
>>>>>>> 4cefcd58c676fa84227e92e368ee84a854e99758
     ...
     for index in openedtabs: #creating another dictionary in the index of the dictionary given by the user  O(n) n:length of openedtabs
        ...
        if title and title.isalnum():      #O(n) n:being invalid input
            url=input("please enter the URL:")
            if url and url.startswith(("www.","http:","https:")):  #O(1)
                nestedtabs={"title":title,"url":url}
            else :
                url=input("please enter the url again or it won't be opened:")  #O(1)
        else :
            print("please input a valid Title!")
...
>>>>>>> 4cefcd58c676fa84227e92e368ee84a854e99758
    ...
    while border<len(openedtabs)-1:  #O(n) n:being the length of the list
      ...
      for i in range (border+1,len(openedtabs)):   #O(n) n:to find the minimum of the minimum element
